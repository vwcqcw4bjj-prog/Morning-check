...
GITHUB_RAW_BASE = "https://raw.githubusercontent.com/vwcqcw4bjj-prog/Morning-check/main"
...
def _fetch_json(filename):
    """GitHubのRaw URLからJSONを取得（5分キャッシュ）"""
    now = time.time()
    if filename in _cache and now - _cache[filename]["ts"] < CACHE_TTL:
        return _cache[filename]["data"]
    try:
        url = "%s/%s" % (GITHUB_RAW_BASE, filename)
        r = requests.get(url, timeout=10)
        if r.status_code == 200:
            data = r.json()
            _cache[filename] = {"data": data, "ts": now}
            return data
    except Exception as e:
        print("[WARN] fetch %s failed: %s" % (filename, e))
    # キャッシュが古くても返す
...
def _market_table(market_data):
    ...
    for r in market_data:
        def fmt_pct(v):
            ...
            return '<td class="num %s">%+.2f%%</td>' % (color, v)
        v = r.get("value")
        cells = "<td>%s</td>" % _e(r.get("name",""))
        cells += '<td class="num">%s</td>' % ("{:,.2f}".format(v) if v is not None else "-")
    ...
    return "<table><thead>%s</thead><tbody>%s</tbody></table>" % (header, "".join(rows))
...
def _headlines_table(headlines):
    ...
    for r in headlines:
        ...
        url = r.get("url", "#")
        rows.append(
            '<tr data-src="%s">' % _e(src) +
            '<td class="dc">%s</td>' % _e(r.get("date","")) +
            '<td><span class="badge" style="--c:%s">%s</span></td>' % (col, _e(src)) +
            '<td><a href="%s" target="_blank" rel="noopener">%s</a></td>' % (_e(url), _e(str(r.get("title",""))[:220])) +
            "</tr>"
        )
    src_btns = "".join(
        '<button class="fbtn" data-src="%s">%s</button>' % (_e(s), _e(s))
        for s in sources
    )
...
def index():
    ...
    data = _fetch_json("headlines.json")
    market_data = _fetch_json("market.json")
...
<h2>データがありません</h2>
<p style="color:#8b949e">しばらくお待ちください。</p>
...
<main>
  ...
  <section>
    <p class="stitle">
      ヘッドライン - 直近7日
      <span class="cnt" id="cnt">HL_COUNT件</span>
    </p>
    <div class="toolbar">
...
</html>""").replace("UPDATED_AT", updated_at
    ).replace("MARKET_HTML", market_html
    ).replace("HL_COUNT", str(count)
    ).replace("SRC_BTNS", src_btns
    ).replace("HL_ROWS", hl_rows)
