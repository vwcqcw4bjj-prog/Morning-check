...
def _market_table(df):
    ...
    for _, r in df[cols].iterrows():
        ...
        cells += (
            ...
            if not pd.isna(v) else '<td class="num">&mdash;</td>'
        )
...
def _headlines_table(df):
    ...
    for _, r in df.iterrows():
        ...
        rows.append(
            ...
            f'<td class="dc">{_e(r.get("date_str",""))}</td>' +
...
def _loading_page():
    return (
        ...
        ".sp{width:36px;height:36px;border:3px solid #30363d;border-top-color:#58a6ff;"
        "border-radius:50%;animation:spin 1s linear infinite;margin:0 auto 20px;}"
        "@keyframes spin{to{transform:rotate(360deg)}}"
        "</style></head><body>"
        "<div class='box'><div class='sp'></div>"
        "<h2>データを取得中です...</h2>"
        "<p>初回は30〜90秒かかります。自動的に更新されます。</p>"
        "</div>"
        "<script>"
        "const check=()=>fetch('/status').then(r=>r.json()).then(d=>{if(!d.running&&d.count>0)location.reload();});"
...
def _build_page(df_week, df_market, updated_at, running):
    ...
    return (
        ...
        f"<span class='ts' id='ts'>更新: {updated_at or '-'}</span>"
        f"<button class='rbtn' id='rbtn' {ref_disabled} onclick='doRefresh()'>{ref_label}</button>"
        "</header>"
        "<main>"
        "<section>"
        "<p class='stitle'>市況データ</p>"
        f"<div class='card'>{market_html}</div>"
        "</section>"
        "<section>"
        f"<p class='stitle'>ヘッドライン - 直近7日 <span class='cnt' id='cnt'>{count}件</span></p>"
        "<div class='toolbar'>"
        "<button class='fbtn on' data-src='ALL'>すべて</button>"
        f"{src_btns}"
        "<input class='search' type='search' id='sq' placeholder='キーワード検索...'>"
        "</div>"
        f"<div class='card'>{hl_html}</div>"
        "</section>"
        "</main>"
        "<div id='toast'></div>"
        "<script>"
