...
def fetch_yahoo_jp_history(code, name):
    """Yahoo Finance Japan 履歴ページから時系列データを取得"""
    ...
    try:
        ...
        if r.status_code != 200:
            print("[YahooJP] %s: status %d" % (name, r.status_code))
        ...
        for tr in table.find_all("tr")[1:]:  # ヘッダーをスキップ
            ...
            close_txt = tds[4].get_text(strip=True).replace(",", "")
            try:
                d = pd.to_datetime(date_txt, format="%Y/%m/%d", errors="coerce")
                v = float(close_txt)
        ...
        print("[YahooJP] %s: %d rows (last=%s)" % (name, len(s), s.index.max().date()))
        return s
