...
def scrape_boj(limit=50):
    ...
    for dl in soup.find_all("dl"):
        for node in dl.find_all(["dt", "dd"]):
            if node.name == "dt":
                current_date = _parse_date(node.get_text(" ", strip=True))
            else:
                for a in node.find_all("a", href=True):
                    ...
                    rows.append({"source": "BOJ", "date": d, "date_str": current_date, "title": title, "url": full, "seq": seq})
                    seq += 1
                    if len(rows) >= limit:
                        return rows
    return rows
...
all_rows = []
for name, fn in [("boj", scrape_boj), ("ppc", scrape_ppc), ("jvca", scrape_jvca), ("chiginkyo", scrape_chiginkyo)]:
    try:
        got = fn()
