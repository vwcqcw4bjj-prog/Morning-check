...
def scrape_wsj(limit=30):
    return parse_rss("https://feeds.content.dowjones.io/public/rss/RSSJapanMarket", "WSJ", limit)
...
def scrape_chiginkyo(limit=100):
    """第一地銀協 - 銀行名をタイトル冒頭に付加"""
    url = "https://www.chiginkyo.or.jp/regional_banks/news/?page_num=50"
    resp = http_get(url)
    if not resp:
        print("[WARN] Chiginkyo: fetch failed")
        return []
    soup = BeautifulSoup(resp.text, "html.parser")
    rows, seen, seq = [], set(), 0
    date_pat = re.compile(r"20\d{2}\.\d{2}\.\d{2}")
    main = soup.find("main") or soup.find("div", id="contents") or soup
    current_bank = ""
    current_date = ""
    for node in main.descendants:
        if not hasattr(node, "get_text"):
            ...
            if not txt:
                continue
            dm = date_pat.search(txt)
            if dm:
                parts = dm.group(0).split(".")
                current_date = "%s-%s-%s" % (parts[0], parts[1], parts[2])
            elif re.search(r"(銀行|信託|金庫)", txt) and len(txt) <= 25:
                current_bank = txt.strip()
            continue
        if node.name == "a":
            href = (node.get("href") or "").strip()
            if not href or href.startswith("#"):
                continue
            if "chiginkyo.or.jp" in href:
                continue
            title = node.get_text(strip=True)
            if not title or len(title) < 5 or href in seen:
                continue
            if _is_future(current_date):
                continue
            seen.add(href)
            bank_prefix = ("[%s] " % current_bank) if current_bank else ""
            d = pd.to_datetime(current_date, errors="coerce") if current_date else None
            rows.append({
                "source": "第一地銀",
                "date": d,
                "date_str": current_date,
                "title": bank_prefix + title,
                "url": href,
            })
            seq += 1
            if len(rows) >= limit:
