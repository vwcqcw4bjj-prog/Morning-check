...
# 既存のmarket.json（Junoが生成）を読み込んでS&P500とJGBをマージ
try:
    with open("market.json", "r", encoding="utf-8") as f:
        existing = json.load(f)
    existing_market = existing.get("market", [])
    # S&P500とJGBのみ上書き
    new_names = {r["name"] for r in market_rows}
    merged = [r for r in existing_market if r["name"] not in new_names]
    merged.extend(market_rows)
    # 順序を固定
    order = ["TOPIX","Nikkei225","S&P500","TOPIX Banks ETF","Japan Post","JP Bank",
             "JGB2Y","JGB5Y","JGB10Y","JGB20Y"]
    merged_sorted = sorted(merged, key=lambda r: order.index(r["name"]) if r["name"] in order else 99)
except Exception as e:
    print("[WARN] merge failed: %s" % e)
    merged_sorted = market_rows

with open("market.json", "w", encoding="utf-8") as f:
    json.dump({
        "updated_at": dt.datetime.now().strftime("%Y/%m/%d %H:%M JST"),
        "market": merged_sorted,
    }, f, ensure_ascii=False, indent=2)

print("[OK] market.json: %d items" % len(merged_sorted))
